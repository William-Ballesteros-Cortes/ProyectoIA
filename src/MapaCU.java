...
public class MapaCU extends Mapa{

    private int acumulado;
    // la posicion 0 es el numero de la nave, 1 los pasos restantes, 2 la fila y 3 columna
    private int[][] naves;

    public MapaCU(int[][] contadorItems,boolean devolverse,List<int[]> moviJugador, List<String> direccion, int acumuladoI, int[][] navesI){
        super(contadorItems, devolverse, moviJugador, direccion);
        this.acumulado = acumuladoI;
    ...
    public MapaCU(MapaCU principal){
        this(principal.getContadorItems(), principal.getDevolverse(), principal.getMoviJugador(), principal.getDireccion(), principal.getAcumulado(), principal.getNaves());
    }

    public void setAcumulado(int acumuladoI){
        this.acumulado = acumuladoI;
    }
